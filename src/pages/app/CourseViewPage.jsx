...
import { apiGetCourses, apiGetProgress, apiSaveProgress } from '../../utils/api'
...
export default function CourseViewPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      apiGetProgress(user.id, courseId)
    ]).then(([c, prog]) => {
      setCourse(c); setProgress(prog); setLoading(false)
    })
  }, [courseId, user?.id])
  ...
  async function toggleComplete(dayId) {
    ...
    setProgress(newProg)
    await apiSaveProgress(user.id, courseId, newProg)
  }
  ...
  return (
    <>
      <SEO
        title={course.title}
        description={`${course.description?.slice(0, 150)} — Free, structured ${course.duration} roadmap on LearnHub.`}
        ...
        {course.weeks.map((week, wi) => (
          <div key={week.id} style={{ marginBottom: 28 }}>
            <div className="week-header">
              ...
              <span style={{ fontFamily: "'Roboto Mono',monospace", fontSize: 11, color: 'var(--text3)', flexShrink: 0 }}>{week.days.filter(d => completed.has(d)).length}/{week.days.length}</span>
            </div>
            {week.days.map(dayId => {
                ...
                <div key={dayId} className={`day-card${isDone ? ' completed' : ''}${isOpen ? ' open' : ''}`}>
                  <div className="day-header" onClick={() => toggleExpand(dayId)}>
                    ...
                    <div className="day-tags-wrap">
                      {day.tags.map(t => <span key={t} className={tagClass(t)} style={{ fontSize: 10 }}>{t}</span>)}
                          ...
                          <ul style={{ listStyle: 'none' }}>
                            {day.resources.map((r, i) => (
                              <li key={i} style={{ padding: '3px 0' }}>
                                <a href={r.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 13, color: 'var(--primary)', textDecoration: 'none', display: 'flex', alignItems: 'flex-start', gap: 6, fontWeight: 500 }}>
