...
import { apiGetCourses, apiUpdateUser, apiEnrollCourse } from '../../utils/api'
...
export default function OnboardingPage() {
  ...
  const pendingCourseId = next?.startsWith('/course/') ? next.replace('/course/', '') : null
  ...
  async function handleComplete() {
    setSaving(true)
    try {
      let updated = await apiUpdateUser(user.id, { bio: bio.trim(), goal, onboardingDone: true })
      if (pendingCourseId && !(updated.enrolledCourses || []).includes(pendingCourseId)) {
        updated = await apiEnrollCourse(user.id, pendingCourseId)
      }
      updateUser(updated)
  ...
  async function handleSkip() {
    try {
      ...
      updateUser({ ...user, onboardingDone: true })
    }
    navigate(next || '/dashboard', { replace: true })
  }
  ...
  return (
    <>
      <SEO title="Welcome to LearnHub" description="Set up your learning profile." path="/onboarding" noindex />
        ...
        <div className="card" style={{ width: '100%', maxWidth: 520, padding: '36px 32px' }}>
          ...
          <div className="form-group">
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {goals.map(g => (
                <button key={g} onClick={() => setGoal(g)} style={{ padding: '7px 14px', borderRadius: 20, fontSize: 12, fontWeight: 500, cursor: 'pointer', border: `1.5px solid ${goal === g ? 'var(--primary)' : 'var(--border)'}`, background: goal === g ? 'var(--primary-light)' : 'transparent', color: goal === g ? 'var(--primary)' : 'var(--text2)', transition: 'all .15s' }}>
                  {g}
                </button>
              ))}
            </div>
          ...
          <div className="form-group">
            ...
            <textarea className="input" rows={3} placeholder="e.g. CS student exploring web dev…" value={bio} onChange={e => setBio(e.target.value)} maxLength={200} style={{ resize: 'vertical', lineHeight: 1.6 }} />
            <div style={{ fontSize: 11, color: 'var(--text3)', textAlign: 'right', marginTop: 4 }}>{bio.length}/200</div>
          </div>

          <button className="btn btn-primary btn-lg" onClick={handleComplete} disabled={saving} style={{ width: '100%', justifyContent: 'center', marginTop: 8 }}>
            {saving ? <span className="spinner" /> : pendingCourse ? 'Complete Setup & Start Course →' : 'Complete Setup →'}
          </button>
          <button className="btn btn-ghost btn-sm" onClick={handleSkip} style={{ width: '100%', justifyContent: 'center', marginTop: 10, color: 'var(--text3)' }}>
            Skip for now
          </button>
        </div>
      </div>
