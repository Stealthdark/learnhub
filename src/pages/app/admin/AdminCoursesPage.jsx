...
import { apiGetCourses, apiSetCourse, apiDeleteCourse, generateId } from '../../../utils/api'
...
export default function AdminCoursesPage() {
  ...
  useEffect(() => { apiGetCourses().then(setCourses) }, [])
  ...
  async function handleSave() {
    ...
    const newCourse = { ...form, id: generateId(), weeks: [], days: [], createdAt: new Date().toISOString() }
  ...
  function extractCourseObjectFromScript(scriptText) {
    const varPatterns = [/(?:const|var|let)\s+(COURSE_DATA)\s*=/, /(?:const|var|let)\s+(\w*COURSE\w*)\s*=/]
    for (const pattern of varPatterns) {
      ...
      for (; i < scriptText.length; i++) {
        ...
        if (inStr) { if (ch === strChar && scriptText[i - 1] !== '\\') inStr = false }
  ...
  function handleImport(e) {
    ...
    reader.onload = async ev => {
      try {
        ...
        if (courseData?.title && Array.isArray(courseData.days)) {
          const imported = { ...courseData, id: courseData.id || generateId(), createdAt: courseData.createdAt || new Date().toISOString() }
          await apiSetCourse(imported)
          setCourses(prev => [...prev, imported])
          showToast(`"${imported.title}" imported — ${imported.days.length} days!`, 'success')
        } else {
          const title = doc.querySelector('title')?.textContent?.replace('Dev. Harshit | ', '').trim() || 'Imported Course'
          const description = doc.querySelector('meta[name="description"]')?.getAttribute('content') || 'Imported from HTML file.'
          const newCourse = { id: generateId(), title, subtitle: '', description, category: '', level: 'Beginner', duration: '', dailyEffort: '', prerequisites: '', outcome: '', weeks: [], days: [], createdAt: new Date().toISOString() }
          await apiSetCourse(newCourse)
          setCourses(prev => [...prev, newCourse])
          showToast(`"${title}" imported (no structured content found).`, 'info')
        }
      } catch (err) {
  ...
  return (
    <>
      <SEO title="Manage Courses" description="Admin course management." path="/admin/courses" noindex />
