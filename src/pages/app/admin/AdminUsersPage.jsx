...
export default function AdminUsersPage() {
  ...
  useEffect(() => {
    if (!selected || (selected.enrolledCourses || []).length === 0) {
      setProgressMap({})
      return
    }
    Promise.all(
      selected.enrolledCourses.map(cid =>
        apiGetProgress(selected.id, cid).then(prog => [cid, prog])
      )
            ...
            <tbody>
              {filtered.map(u => (
                <tr key={u.id} style={{ background: selected?.id === u.id ? 'var(--primary-light)' : '' }}>
                  ...
                  <td>
                    <span className={`chip ${u.role === 'admin' ? 'chip-amber' : 'chip-blue'}`} style={{ fontSize: 11 }}>
                      {u.role}
                    </span>
                  </td>
                  <td>
                    {u.verified
                      ? <span className="chip chip-green" style={{ fontSize: 11 }}>✓</span>
                      : <span className="chip chip-red" style={{ fontSize: 11 }}>Pending</span>
                    }
                  </td>
                  <td>
          ...
          <div className="user-detail-panel">
            <div className="card" style={{ padding: 20, marginBottom: 12 }}>
              ...
              {[
                ...
                ['Joined', new Date(selected.joinedAt).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })],
              ].map(([k, v]) => (
