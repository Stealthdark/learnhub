...
import AvatarEl from '../../components/shared/AvatarEl'
import { useAuth } from '../../hooks/useAuth'
import { useToast } from '../../hooks/useToast'
import { apiGetCourses, apiGetProgress, apiUpdateUser, apiChangePassword } from '../../utils/api'
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    Promise.all(enrolled.map(cid => apiGetProgress(user.id, cid).then(prog => [cid, prog]))).then(entries => setProgressMap(Object.fromEntries(entries)))
  ...
  async function handleProfileSave() {
    ...
    try {
      ...
      updateUser(updated); showToast('Profile updated!', 'success'); setErrors({})
  ...
  async function handlePasswordSave() {
    ...
    try {
      ...
      setErrors({})
    } catch (err) { setErrors({ currentPassword: err.message || 'Password change failed' }) }
  }
  ...
  function handleAvatar(e) {
    ...
    reader.onload = async ev => {
      try {
        ...
        updateUser(updated); showToast('Avatar updated!', 'success')
      } catch (err) { showToast(err.message || 'Failed to update avatar', 'error') }
    }
  ...
  return (
    <>
      <SEO title="Account Settings" description="Manage your LearnHub profile, security, and activity." path="/profile" noindex />
      ...
      <div className="page-pad" style={{ maxWidth: 680 }}>
            ...
            <div className="card" style={{ padding: 20, marginBottom: 16 }}>
              ...
              {[['currentPassword', 'Current Password', 'password'], ['newPassword', 'New Password', 'password'], ['confirmNewPassword', 'Confirm New Password', 'password']].map(([k, lbl, type]) => (
                <div className="form-group" key={k}><label className="label">{lbl}</label>
                  <input className={`input${errors[k] ? ' error' : ''}`} type={type} value={form[k]} onChange={e => set(k, e.target.value)} autoComplete="new-password" />
        ...
        {tab === 'activity' && (
          <div>
            ...
            {enrolled.length === 0 ? (
              <div className="card" style={{ padding: 28, textAlign: 'center', color: 'var(--text2)' }}>No courses enrolled yet.</div>
            ) : enrolled.map(cid => {
